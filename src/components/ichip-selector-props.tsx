...
const AnimatedFlatList =
  Animated.createAnimatedComponent<FlatListProps<Data>>(FlatList);
...
const IChipSelector = forwardRef<Ref, IChipSelectorProps>(
  (
    {data, contentContainerStyle, style, onReset, selectedIds, onSelect},
    ref,
  ) => {
    ...
    const renderChip: ListRenderItem<Data> = ({item, index}) => {
      ...
      return (
        <Animated.View
          key={String(rendered) + String(index)}
          style={{width: !isSelected ? undefined : undefined}}>
          <IChip
            onPress={handleChange(onSelect?.bind('', item.id))}
            title={item.name || ''}
    ...
    const renderCell = React.useCallback((props: CellRendererProps<Data>) => {
      return (
        <Animated.View
          {...props}
          key={props.cellKey}
